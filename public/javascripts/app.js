...
var appendToPage = function() {
  for (var i = 0; i < wordLetters.length; i++) {
    ...
    for (var j = 0; j < currentWord.length; j++) {
      ...
      var image = $("<img>").attr("src", currentWord[j]).css('height', getRandomInt(100, 150) + "px");
      image.css('border-radius', getRandomInt(0, 50) + "%");
      image.css('width', getRandomInt(100, 150) + "px");
      image.css('-webkit-transform', 'rotate('+ getRandomInt(0, 7)+'deg)')
      $(div).append(image);
    }
  }
};
